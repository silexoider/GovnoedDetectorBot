...
public abstract class BaseHandler<TKey, TRes, TSel, TArg> {
    ...
    protected void init(Iterable<Map.Entry<TKey, BiFunction<TSel, TArg, TRes>>> entries) {
        for (Map.Entry<TKey, BiFunction<TSel, TArg, TRes>> entry : entries) {
            container.insert(entry.getKey(), entry.getValue());
        }
    }

    protected TRes handle(TSel selector, TArg argument) {
